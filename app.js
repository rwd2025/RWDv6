...
const Modules = [
  ...
  ["Work Orders","Job workflow","clock","▤"],["Invoices","Paper bill","invoice","▥"],["Time Clock","3 live jobs","clock","◷"],["Camera / OCR","Scan VIN / part","scan","📷"],["DOT / GPS","Dealer location","gps","⌖"],
  ["Repair Memory","Saved fixes","repair","🧠"],["Suppliers","FleetPride etc.","suppliers","◆"],["AI Assistant","Master command","ai","RW"],["Reports","Income + labor","reports","▥"],["Settings","Themes / debug","settings","⚙"],["Emergency","Roadside flow","emergency","⚠"]
];
...
const RWDModuleRegistry = {
  ...
  workorders: {
    ...
    render: () => `
      <div class="pro-panel">
        <div class="pro-header"><h3>WORK ORDER BUILDER</h3><span class="status-pill">CCC READY</span></div>
        ...
        <button class="pro-btn-gold" onclick="RWDModuleActions.saveWorkOrder()">SAVE WORK ORDER</button>
        <div class="view-area" id="woOutput">Work order ready. Can link to VIN, job clock, photos, and invoice.</div>
      </div>`
  },
  ...
  gps: {
    ...
    render: () => `
      <div class="pro-panel">
        <div class="pro-header"><h3>DOT / GPS</h3><span class="status-pill">LOCATION SHELL</span></div>
        <button class="pro-btn-gold" onclick="window.open('https://www.google.com/maps/search/heavy+duty+truck+parts+near+me','_blank')">OPEN NEARBY PARTS DEALERS</button>
        <div class="view-area">GPS route, supplier location, DOT inspection, and roadside dispatch hooks live here.</div>
      </div>`
  }
};
...
function boot(){
  ...
  $("masterCommandInput").addEventListener("focus", () => AiDrawer.open());
  $("masterCommandInput").addEventListener("keydown", e => { if(e.key==="Enter"){ AiDrawer.open(); $("aiDrawerInput").value=e.target.value; AiDrawer.ask(); e.target.value=""; }});
...
/* ===== FINAL LOCKED COMMAND CENTER PATCH: routes/actions only ===== */
const RWD_FINAL_ROUTES={home:'home',clock:'clock',truck:'truck',parts:'parts',invoice:'invoice',quote:'invoice',quotes:'invoice',ai:'ai',assistant:'ai',doctor:'faults',faults:'faults',repair:'repair',repairhud:'repair',workorders:'workorders',emergency:'emergency',roadside:'emergency',settings:'settings',more:'settings',debug:'debug',finance:'finance',reports:'reports',gps:'gps',suppliers:'suppliers',memory:'memory',scan:'scan',generic:'generic'};
function RWD_finalResolve(id){id=String(id||'home').trim();return RWD_FINAL_ROUTES[id]||id;}
function RWD_finalGo(id){id=RWD_finalResolve(id);const target=document.getElementById(id)||document.getElementById('generic'); if(!target)return false; document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active')); target.classList.add('active'); document.querySelectorAll('.bottom-nav button').forEach(b=>b.classList.toggle('active',RWD_finalResolve(b.dataset.nav)===id)); document.getElementById('sideMenu')?.classList.remove('open'); window.scrollTo({top:0,behavior:'smooth'}); return true;}
if(typeof NavigationManager!=='undefined'){NavigationManager.navigate=function(id){return RWD_finalGo(id)}}
function RWD_finalLockRoutes(){const bottom=document.getElementById('bottomNav')||document.querySelector('.bottom-nav'); if(bottom){const fixed=['home','faults','parts','workorders','settings']; bottom.querySelectorAll('button').forEach((b,i)=>{if(fixed[i]) b.dataset.nav=fixed[i]})} document.querySelectorAll('[data-nav]').forEach(btn=>{const txt=(btn.textContent||'').replace(/\s+/g,' ').trim().toUpperCase(); if(txt.includes('BACK HOME')||txt==='HOME COMMAND CENTER')btn.dataset.nav='home'; if(txt.includes('AI ASSISTANT')||txt.includes('MASTER COMMAND'))btn.dataset.nav='ai'; if(txt.includes('OEM PARTS'))btn.dataset.nav='parts'; if(txt.includes('WORK ORDERS'))btn.dataset.nav='workorders'; if(txt.includes('EMERGENCY'))btn.dataset.nav='emergency';});}
document.addEventListener('click',function(e){const nav=e.target.closest('[data-nav]'); if(nav){e.preventDefault(); e.stopPropagation(); RWD_finalGo(nav.dataset.nav); return false;}},true);
const RWDPartsFinal={answer(q){q=(q||'').trim();const l=q.toLowerCase(); if(!q)return 'Ask for a part, part number, VIN/ESN/CPL, or upload a photo.'; if(l.includes('x15')&&l.includes('water pump'))return '<b>2019 Cummins X15 water pump</b><br>Show likely pump options first, estimated market range, quick suppliers, job kit, then ask last 8 VIN for exact verified part number.<br><br><b>Job kit:</b> pump, gasket/seal, ELC coolant, clamps, belt inspection, pressure test.'; if((l.includes('dd13')||l.includes('dd 13'))&&l.includes('water pump'))return '<b>2017 Detroit DD13 water pump</b><br>Likely DD13/DD15 pump family. Mechanical vs variable-speed must be verified by last 8 VIN/ESN.<br><br><b>Job kit:</b> pump, seal/gasket, coolant, clamps, belt inspection, pressure test.'; if(l.includes('t680')&&(l.includes('coolant')||l.includes('sleeper')||l.includes('arrow')))return '<b>Kenworth T680 sleeper coolant fitting</b><br>Likely sleeper heater coolant check/flow valve. Arrow points coolant flow toward sleeper heater circuit. Upload photo/year/engine to narrow fitting style.'; return '<b>Parts lookup:</b> '+q.replace(/[<>]/g,'')+'<br>Show likely OEM/aftermarket options, cross refs, fitment, estimated cost, job kit, then ask last 8 VIN only for exact ordering.'},run(){const q=document.getElementById('partNum')?.value||document.getElementById('masterCommandInput')?.value||''; const out=document.getElementById('partsOut'); if(out)out.innerHTML=this.answer(q);}};
document.addEventListener('click',function(e){const a=e.target.closest('[data-action]')?.dataset.action; if(!a)return; if(a==='add-part'||a==='parts-ai-search'){setTimeout(()=>RWDPartsFinal.run(),0)} if(a==='ask-ai'){const input=document.getElementById('aiInput'); const chat=document.getElementById('chatBox'); if(input&&chat&&input.value.trim()){chat.innerHTML+='<div class="user-msg">'+input.value.replace(/[<>]/g,'')+'</div><div class="ai-msg">'+RWDPartsFinal.answer(input.value)+'</div>'; input.value='';}}},true);
document.addEventListener('DOMContentLoaded',()=>{RWD_finalLockRoutes(); RWD_finalGo('home')}); setTimeout(()=>{RWD_finalLockRoutes(); RWD_finalGo('home')},350);
