...
/* ===== V5.4 TOTAL BUTTON ROUTER / AUDIT FIX ===== */
const RWD_SCREEN_ROUTES = {
  home:"home",
  clock:"clock",
  truck:"truck",
  parts:"parts",
...
function RWD_resolveRoute(id){
  id = String(id || "home").trim();
  return RWD_SCREEN_ROUTES[id] || id;
}

function RWD_showScreen(id){
  id = RWD_resolveRoute(id);
  const target = document.getElementById(id);
  if(!target){
    console.warn("Missing route target:", id);
    return false;
  }
...
// Override old router completely
if(typeof NavigationManager !== "undefined"){
  NavigationManager.navigate = function(id){ RWD_showScreen(id); };
}

// Catch every nav button, even if old listeners are wrong.
...
// One final hard assignment after dynamic module build
function RWD_fixAllRoutes(){
  const byText = [
    [/^HOME$/i, "home"],
    [/DOCTOR|AI BRAIN|AI ASSISTANT|ASK RWD|RW AI/i, "ai-full"],
    [/PARTS|OEM PARTS|PARTS LOOKUP/i, "parts"],
    [/SMART QUOTES|QUOTE/i, "invoice"],
    [/REPAIR ORDERS|WORK ORDERS|REPAIR\b/i, "workorders"],
    [/EMERGENCY|ROADSIDE|ROAD CALL|ROAD$/i, "emergency"],
    [/TIME CLOCK|CLOCK/i, "clock"],
    [/SETTINGS|THEMES/i, "settings"],
    [/DEBUG/i, "debug"]
  ];
  document.querySelectorAll("[data-nav]").forEach(btn => {
    const text = (btn.textContent || "").replace(/\s+/g," ").trim();
    for(const [rx,id] of byText){
      if(rx.test(text)){ btn.dataset.nav = id; break; }
    }
  });
  // Bottom nav absolute truth
  const bottom = document.querySelector("#bottomNav");
  if(bottom){
    const fixed = ["home","ai-full","parts","workorders","emergency"];
    bottom.querySelectorAll("button").forEach((btn,i)=>{ if(fixed[i]) btn.dataset.nav = fixed[i]; });
  }
}
...
setTimeout(RWD_fixAllRoutes, 300);
setTimeout(()=>RWD_showScreen("home"), 350);


/* ===== V5.4 ACTION FALLBACKS ===== */
document.addEventListener("click", function(e){
  const btn = e.target.closest("[data-action]");
  if(!btn) return;
  const action = btn.dataset.action;
  if(action === "parts-ai-search"){ e.preventDefault(); PartsBrain.run(); }
  if(action === "parts-mode-unknown"){ e.preventDefault(); PartsBrain.setMode("unknown"); }
  if(action === "parts-mode-number"){ e.preventDefault(); PartsBrain.setMode("number"); }
  if(action === "parts-mode-cross"){ e.preventDefault(); PartsBrain.setMode("cross-reference"); }
  if(action === "parts-mode-fitment"){ e.preventDefault(); PartsBrain.setMode("verify fitment"); }
  if(action === "open-ai-drawer"){ e.preventDefault(); RWD_showScreen("ai-full"); }
  if(action === "ai-full-back"){ e.preventDefault(); RWD_showScreen("home"); }
  if(action === "ai-full-clear"){ e.preventDefault(); const c=document.getElementById("aiFullChat"); if(c)c.innerHTML='<div class="ai-msg">Chat cleared.</div>'; }
}, true);


/* ===== V5.4.1 FULL BUTTON FALLBACKS ===== */
document.addEventListener("click", function(e){
  const b = e.target.closest("button, [role='button']");
  if(!b) return;
  const text = (b.textContent || "").replace(/\s+/g," ").trim().toUpperCase();
  if(text.includes("BACK HOME") || text === "BACK" || text.includes("← BACK") || text.includes("EXIT")){
    e.preventDefault();
    e.stopPropagation();
    if(typeof RWD_showScreen === "function") RWD_showScreen("home");
    return false;
  }
}, true);


/* ===== V5.4.1 EXTRA ACTION DEAD-CLICK GUARD ===== */
document.addEventListener("click", function(e){
  const btn=e.target.closest("[data-action]");
  if(!btn) return;
  const action=btn.dataset.action;
  if(action==="parts-upload-photo"){ e.preventDefault(); RWD_showScreen("ai-full"); document.getElementById("aiFullUpload")?.click(); }
  if(action==="parts-take-photo"){ e.preventDefault(); RWD_showScreen("ai-full"); document.getElementById("aiFullCamera")?.click(); }
  if(action==="ai-full-send"){ e.preventDefault(); const chat=document.getElementById("aiFullChat"); const input=document.getElementById("aiFullInput"); if(chat&&input){chat.innerHTML += '<div class="user-msg">'+input.value+'</div><div class="ai-msg">RWD AI frontend ready. Backend AI plugs in next.</div>'; input.value='';} }
  if(action==="ai-full-voice" || action==="voice" || action==="ai-start-listening"){ e.preventDefault(); alert("Voice frontend ready. Browser permission/backend plugs in next."); }
  if(action==="ai-speak-last"){ e.preventDefault(); if(window.speechSynthesis){ speechSynthesis.speak(new SpeechSynthesisUtterance("Rolling Wrench AI ready.")); } }
  if(action==="build-smart-quote" && typeof SmartQuoteEngine!=="undefined"){ e.preventDefault(); SmartQuoteEngine.build(); }
  if(action==="quote-copy-customer" && typeof SmartQuoteEngine!=="undefined"){ e.preventDefault(); SmartQuoteEngine.copyCustomer(); }
  if(action==="quote-to-workorder" && typeof SmartQuoteEngine!=="undefined"){ e.preventDefault(); SmartQuoteEngine.toWorkOrder(); }
  if(action==="quote-to-invoice" && typeof SmartQuoteEngine!=="undefined"){ e.preventDefault(); SmartQuoteEngine.toInvoice(); }
}, true);


/* ===== V5.5 CLEAN REPO ABSOLUTE ROUTER ===== */
const RWD_ROUTES={home:"home",clock:"clock",truck:"truck",parts:"parts",invoice:"invoice",quote:"invoice",quotes:"invoice",workorders:"workorders",repair:"workorders",repairorders:"workorders",emergency:"emergency",roadside:"emergency",settings:"settings",debug:"debug",finance:"finance",reports:"reports",suppliers:"suppliers",memory:"memory",faults:"faults",scan:"scan",gps:"gps",ai:"ai-full",assistant:"ai-full",doctor:"ai-full","ai-full":"ai-full"};
function RWD_resolveRoute(id){id=String(id||"home").trim();return RWD_ROUTES[id]||id;}
function RWD_showScreen(id){id=RWD_resolveRoute(id);const t=document.getElementById(id);if(!t){console.warn("Route target missing:",id);return false;}document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active"));t.classList.add("active");document.querySelectorAll("[data-nav]").forEach(b=>b.classList.toggle("active",RWD_resolveRoute(b.dataset.nav)===id));window.scrollTo(0,0);return true;}
if(typeof NavigationManager!=="undefined"){NavigationManager.navigate=function(id){return RWD_showScreen(id);};}
function RWD_fixRoutes(){const bottom=document.querySelector("#bottomNav");if(bottom){["home","ai-full","parts","workorders","emergency"].forEach((id,i)=>{const b=bottom.querySelectorAll("button")[i];if(b)b.dataset.nav=id;});}}
document.addEventListener("click",function(e){const nav=e.target.closest("[data-nav]");if(nav){e.preventDefault();e.stopPropagation();RWD_showScreen(nav.dataset.nav);return false;}},true);
document.addEventListener("click",function(e){const btn=e.target.closest("[data-action]");if(!btn)return;const a=btn.dataset.action;if(a==="parts-ai-search"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.run();}if(a==="parts-mode-unknown"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("unknown");}if(a==="parts-mode-number"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("number");}if(a==="parts-mode-cross"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("cross-reference");}if(a==="parts-mode-fitment"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("verify fitment");}if(a==="open-ai-drawer"){e.preventDefault();RWD_showScreen("ai-full");}if(a==="ai-full-clear"){e.preventDefault();const c=document.getElementById("aiFullChat");if(c)c.innerHTML='<div class="ai-msg">Chat cleared.</div>';}if(a==="ai-full-send"){e.preventDefault();const input=document.getElementById("aiFullInput"),chat=document.getElementById("aiFullChat");if(input&&chat){chat.innerHTML+='<div class="user-msg">'+input.value+'</div><div class="ai-msg">RWD AI frontend ready. Backend plugs in next.</div>';input.value='';}}if(a==="build-smart-quote"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.build();}if(a==="quote-copy-customer"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.copyCustomer();}if(a==="quote-to-workorder"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.toWorkOrder();}if(a==="quote-to-invoice"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.toInvoice();}},true);
document.addEventListener("DOMContentLoaded",()=>{RWD_fixRoutes();RWD_showScreen("home");});setTimeout(()=>{RWD_fixRoutes();RWD_showScreen("home");},350);


/* ===== V5.6 LOCKED LAYOUT ROUTE PATCH ===== */
function RWD_lockedResolve(id){
  const map={home:"home",clock:"clock",parts:"parts",invoice:"invoice",quotes:"invoice",quote:"invoice",workorders:"workorders",repair:"workorders",repairorders:"workorders",emergency:"emergency",roadside:"emergency",settings:"settings",debug:"debug",ai:"ai-full",assistant:"ai-full",doctor:"ai-full","ai-full":"ai-full",truck:"truck",scan:"scan",finance:"finance",reports:"reports",gps:"gps",suppliers:"suppliers",memory:"memory",faults:"faults"};
  return map[String(id||"home").trim()]||id;
}
function RWD_lockedShow(id){
  id=RWD_lockedResolve(id);
  const target=document.getElementById(id);
  if(!target)return false;
  document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active"));
  target.classList.add("active");
  document.querySelectorAll("[data-nav]").forEach(b=>b.classList.toggle("active",RWD_lockedResolve(b.dataset.nav)===id));
  window.scrollTo(0,0);
  return true;
}
if(typeof NavigationManager!=="undefined"){NavigationManager.navigate=function(id){return RWD_lockedShow(id);};}
document.addEventListener("click",function(e){
  const nav=e.target.closest("[data-nav]");
  if(nav){e.preventDefault();e.stopPropagation();RWD_lockedShow(nav.dataset.nav);return false;}
},true);
document.addEventListener("DOMContentLoaded",()=>RWD_lockedShow("home"));
setTimeout(()=>RWD_lockedShow("home"),300);

/* ===== V6 APP STORE QUALITY ROUTER ===== */
const RWD_APP_ROUTES={home:"home",clock:"clock",truck:"truck",parts:"parts",invoice:"invoice",quote:"invoice",quotes:"invoice",workorders:"workorders",repair:"workorders",repairorders:"workorders",emergency:"emergency",roadside:"emergency",settings:"settings",debug:"debug",finance:"finance",reports:"reports",suppliers:"suppliers",memory:"memory",faults:"faults",scan:"scan",gps:"gps",ai:"ai-full",assistant:"ai-full",doctor:"ai-full","ai-full":"ai-full",generic:"generic"};
function RWD_route(id){id=String(id||"home").trim();return RWD_APP_ROUTES[id]||id;}
function RWD_go(id){id=RWD_route(id);const t=document.getElementById(id);if(!t){console.warn("Missing screen route:",id);return false;}document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active"));t.classList.add("active");document.querySelectorAll("[data-nav]").forEach(b=>b.classList.toggle("active",RWD_route(b.dataset.nav)===id));window.scrollTo(0,0);return true;}
if(typeof NavigationManager!=="undefined"){NavigationManager.navigate=function(id){return RWD_go(id);};}
function RWD_lockRoutes(){const bottom=document.querySelector("#bottomNav");if(bottom){["home","ai-full","parts","workorders","emergency"].forEach((id,i)=>{const b=bottom.querySelectorAll("button")[i];if(b)b.dataset.nav=id;});}}
document.addEventListener("click",function(e){const nav=e.target.closest("[data-nav]");if(nav){e.preventDefault();e.stopPropagation();RWD_go(nav.dataset.nav);return false;}},true);
document.addEventListener("click",function(e){const btn=e.target.closest("[data-action]");if(!btn)return;const a=btn.dataset.action;if(a==="open-ai-drawer"){e.preventDefault();RWD_go("ai-full");}if(a==="ai-full-clear"){e.preventDefault();const c=document.getElementById("aiFullChat");if(c)c.innerHTML='<div class="ai-msg">Chat cleared.</div>';}if(a==="ai-full-send"){e.preventDefault();const input=document.getElementById("aiFullInput"),chat=document.getElementById("aiFullChat");if(input&&chat){chat.innerHTML+='<div class="user-msg">'+input.value.replace(/[<>]/g,"")+'</div><div class="ai-msg">RWD AI frontend ready. Backend plugs in next.</div>';input.value='';}}if(a==="parts-ai-search"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.run();}if(a==="parts-mode-unknown"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("unknown");}if(a==="parts-mode-number"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("number");}if(a==="parts-mode-cross"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("cross-reference");}if(a==="parts-mode-fitment"&&typeof PartsBrain!=="undefined"){e.preventDefault();PartsBrain.setMode("verify fitment");}if(a==="build-smart-quote"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.build();}if(a==="quote-copy-customer"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.copyCustomer();}if(a==="quote-to-workorder"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.toWorkOrder();}if(a==="quote-to-invoice"&&typeof SmartQuoteEngine!=="undefined"){e.preventDefault();SmartQuoteEngine.toInvoice();}},true);
document.addEventListener("DOMContentLoaded",()=>{RWD_lockRoutes();RWD_go("home");});
setTimeout(()=>{RWD_lockRoutes();RWD_go("home");},350);
